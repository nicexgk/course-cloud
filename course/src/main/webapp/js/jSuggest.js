            ...
            if (this.value.length >= opts.minchar && $.trim(this.value) != "Search Terms") {
                ...
                // new query detected
                    ...
                    if (opts.data == '')
                        ...
                        keys = encodeURIComponent($(this).val());
                    //alert(keys);
                    // optimize server performance by loading at intervals
                    setTimeout(function () {
                        var searchText = {};
                        searchText.text = keys;
                        searchText.page = 0;
                        searchText.size = 5;
                        $.ajax({
                            type: opts.type,
                            ...
                            success: function (msg) {
                                ...
                                for (var i in msg) {
                                    var course = msg[i];
                                    var li = '<li class="search-li"><a target="_blank" href="/course/page/" ' + course.courseId + '>' +
                                        '<span class="search-course-name">' + course.courseName + '</span>' +
                                        '<span class="search-teacher">' + course.courseTeacher.userName + '</span>' +
                                        '</a></li>';
                                    content += li;
                                }
                                if (content == ""){
                                    content = '<li class="search-li"><a target="_blank" href="javascript:;">' +
                                        '<span class="search-course-name">没有查找到您要搜索的的课程。。。</span>' +
                                        '<span class="search-teacher"></span>' +
                                        '</a></li>';
                                }
                                $(jC).html(content);
